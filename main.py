...
#zjisteni zda ctverecky sa daji spojit
    ...
    def move(self, dir_x, dir_y):
        ...
        for board_x, board_y in all_cells(dir_x > 0, dir_y > 0):
            ...
            if not tile:
                continue
            x, y = board_x, board_y
#pokud se to muze pohnout, pohne
    ...
    def resize(self, *args):
        # Nastaví velikost buňky v závislosti na šířce a výšce widgetu minus odsazení
        self.cell_size = (.25 * (self.width - 5 * spacing),) * 2

        # Vyčistí kreslící plátno předtím nakreslené prvky
        self.canvas.before.clear()

        # Nakreslí pozadí herního plánu
        ...
        # Pokud není přítomné žádné herní pole (self.b), vrátí funkci
        if not self.b:
            return

        # Pro každou buňku na herním plánu
        for board_x, board_y in all_cells():
            tile = self.b[board_x][board_y]
            # Pokud je v buňce nějaký dlaždicový objekt (tile)
            if tile:
                # Změní velikost dlaždice na základě nové velikosti buňky
                tile.resize(pos=self.cell_pos(board_x, board_y), size=self.cell_size)

    # Nastavení událostí on_pos a on_size tak, aby volaly metodu resize
    on_pos = resize
    on_size = resize
